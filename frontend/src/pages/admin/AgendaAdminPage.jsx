...
import AgendaTable from "../../components/admin/agenda/AgendaTable";
...
const AgendaAdminPage = () => {
  ...
  const handleSave = async (formData) => {
    try {
      if (editingData) {
        await agendaApi.update(editingData.id, formData);
        showNotification("Agenda berhasil diperbarui.", "edit");
      } else {
        await agendaApi.create(formData);
        showNotification("Agenda baru berhasil ditambahkan.", "add");
      }
      fetchData();
      handleCloseModal();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Apakah Anda yakin ingin menghapus agenda ini?")) {
      try {
        ...
        showNotification("Agenda berhasil dihapus.", "delete");
        fetchData();
      } catch (error) {
